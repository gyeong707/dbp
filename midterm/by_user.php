<?php
  $link = mysqli_connect('localhost', 'root', 'exam', 'midterm', 3307);
  if(mysqli_connect_errno()){
        echo "MariaDB 접속에 실패했습니다. 관리자에게 문의하세요.";
        echo "<br>";
        echo mysqli_connect_error();
        exit();
    }

    // initial value
    $totalRowNum = 1600000;
    $query = "
    SELECT * FROM twitter LIMIT 10";
    $senti_query = "
    SELECT target sentiment, count(*) cnt FROM twitter GROUP BY target";
    $user = "Enter the user name";

    // Select User
    if(isset($_POST['user'])) {
        $user = mysqli_real_escape_string($link, $_POST['user']);
        $subquery = "SELECT count(*) FROM twitter WHERE user='{$user}'";
        $subresult = mysqli_query($link, $subquery);
        $subrow = mysqli_fetch_array($subresult);
        $totalRowNum = $subrow['count(*)'];
        $query =  "SELECT * FROM twitter WHERE user = '{$user}' LIMIT 10";
        $senti_query = "
        SELECT target sentiment, count(*) cnt FROM TWITTER WHERE USER='{$user}' group by target;";
    } 

    // Moving Page Event + User
    $currentPage = 1;
    if (isset($_GET["currentPage"])) {
        $currentPage = $_GET["currentPage"];
        // filtering by user name
        if ($_GET['user'] != "Enter the user name"){
            $user = $_GET['user'];
            $subquery = "
            SELECT count(*) FROM twitter WHERE user='{$user}'";
            $subresult = mysqli_query($link, $subquery);
            $subrow = mysqli_fetch_array($subresult);
            $totalRowNum = $subrow['count(*)'];
            $rowPerPage = 10; 
            $begin = ($currentPage -1) * $rowPerPage;
            $query = "
            SELECT * FROM TWITTER WHERE user='{$user}' LIMIT ".$begin.",".$rowPerPage."";
            $senti_query = "
            SELECT target sentiment, count(*) cnt FROM TWITTER WHERE USER='{$user}' group by target;";
        }
        // not filtering by user name
        else {
            $subquery = "
            SELECT count(*) FROM twitter";
            $subresult = mysqli_query($link, $subquery);
            $subrow = mysqli_fetch_array($subresult);
            $totalRowNum = $subrow['count(*)'];
            $rowPerPage = 10; 
            $begin = ($currentPage -1) * $rowPerPage;
            $query = "
            SELECT * FROM TWITTER LIMIT ".$begin.",".$rowPerPage."";
            $senti_query = "
            SELECT target sentiment, count(*) cnt FROM TWITTER group by target;";
        }
    }

    $result = mysqli_query($link, $query);
    $twit_info = "";

    while ($row = mysqli_fetch_array($result)) {
        $filtered = array(
          'id' => htmlspecialchars($row['id']),
          'target' => htmlspecialchars($row['target']),
          'date' => htmlspecialchars($row['date']),
          'user' => htmlspecialchars($row['user']),
          'text' => htmlspecialchars($row['text'])
        );
          $twit_info .= '<tr>';
          $twit_info .= '<td>'.$filtered['id'].'</td>';
          $twit_info .= '<td>'.$filtered['target'].'</td>';
          $twit_info .= '<td>'.$filtered['date'].'</td>';
          $twit_info .= '<td>'.$filtered['user'].'</td>';
          $twit_info .= '<td>'.$filtered['text'].'</td>';
          $twit_info .= '</tr>';
      }

    // Summary Table
    $senti_result = mysqli_query($link, $senti_query);
    $senti_info = "";
    while($senti_row = mysqli_fetch_array($senti_result)) {
        $senti = array(
            'sentiment' => htmlspecialchars($senti_row['sentiment']),
            'cnt' => htmlspecialchars($senti_row['cnt'])
        );
        $senti_info .= '<tr>';
        $senti_info .= '<td>'.$senti['sentiment'].'</td>';
        $senti_info .= '<td>'.$senti['cnt'].'</td>';
        $senti_info .= '</tr>';
    }

     // 할당된 리소스 해제
    mysqli_free_result($result);
    mysqli_free_result($senti_result);
    mysqli_close($link);

?>

<!DOCTYPE html>
<html>
<head>
    <meta charset='utf-8'>
    <title> Tweets categorized by user</title>
    <link rel=stylesheet href="style.css" type="text/css">

    <style></style>

    <body>
        <div id="report">
            <img src="brain.png" id="report_cover">
            <span id="report_title"><a href='index.php'>DBP Midterm Report</a></span>
            <h3 id="report_name">20160965 통계학과 강미경</h3> 
            <h3 id="report_top">Sungshin Women's University</h3>

            <!-- Analysis Info -->
            <div id="report_inner">
            <div class="section">
                <h1 class="section_title">Analysis 2. 특정 사용자가 쓴 글 조회하기</h1>
                <hr>
                <span class="section_dataset_info">
                특정 사용자의 트윗만을 확인할 수 있습니다. <br><br>
                이를 바탕으로 개인화된 맞춤형 분석이 가능합니다.<br><br>
                아래 표에서는 특정 사용자가 작성한 트윗의 감성분포를 확인할 수 있습니다.<br>
                </span>

                <!-- Summary Table -->
                <table class="summary_dataset">
                    <tr>
                    <th>sentiment</th>
                    <th>cnt</th>
                    </tr>
                    <?=$senti_info?>
                </table>
                <br>
                <span class="section_dataset_info">
                    0 : Negative  |  4 : Positive <br>
                </span>
                <br><br><hr>

                <!-- Dataset Table -->
                <h2 class="section_dataset_subtitle">[Tweets categorized by user]</h2>
                <div class="analysis_form">
                    <form action="by_user.php" method="POST">
                        <label>다른 유저의 글 검색: </label>
                        <input type="text" name="user" placeholder="<?=$user?>">
                        <input type="submit" value="submit"><br><br>
                    </form>
                </div>
                <div class="section_rownum">총 <?=$totalRowNum?> 개의 행이 검색되었습니다.</div>
                <table class="dataset">
                    <tr>
                    <th>id</th>
                    <th>target</th>
                    <th>date</th>
                    <th>user</th>
                    <th>text</th>
                    </tr>
                    <?= $twit_info ?>
                </table>

                <!-- Buttons -->
                <form action='by_user.php' method="GET" class="form_buttons">
                    <input type="hidden" name="user" value='<?=$user?>'>
                    <input type="hidden" name="currentPage" value='<?=($currentPage-1)?>'>
                    <input type="submit" name="back" value="Back">
                </form>
                
                <form action='by_user.php' method="GET" class="form_buttons">
                    <input type="hidden" name="user" value='<?=$user?>'>
                    <input type="hidden" name="currentPage" value='<?=($currentPage+1)?>'>
                    <input type="submit" name="next" value="Next">
                </form>

                <br>
                <div class="back_to">
                <a href="by_user.php">처음으로 돌아가기</a><br><br>
                    <a href="index.php">메인으로 돌아가기</a>
                </div>
            </div>
        </div>
    </body>
</head>
</html>
